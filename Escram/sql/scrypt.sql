...
create table Invoice (id varchar(255) not null, dataApertura datetime(6) not null, dataScadenza datetime(6) not null, indirizzoDst varchar(255), indirizzoSrc varchar(255), invoiceId varchar(255) not null, statoDst enum ('ANNULLATO','CONFERMATO','IN_ATTESA') not null, statoSrc enum ('ANNULLATO','CONFERMATO','IN_ATTESA') not null, status varchar(255) not null, usdAmount float(53) not null, primary key (id)) engine=InnoDB;
...
insert into cliente values ('mario.rossi@gmail.com', false, 'Rossi', 'Mario', 'Password01$', 'COMPRATORE');